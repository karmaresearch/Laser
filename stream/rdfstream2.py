...
class RDFStream2(Stream):
	def __init__(self, nTriples):
		super(RDFStream2, self).__init__()
		self.timeLine = {
			"startTime": 0,
			"endTime"  : 999999999,
		}
	...
	def get(self, t):
		...
		for p, i in self.predicates.items():
			...
			for c in range(0, i[0]):
				...
				row.append("http://example.org/stream/class#c" + str(const))
				row.append("http://example.org/stream/class#c" + str(const + 1))
				r[p].add(tuple(row))
				row = [t, self.timeLine["endTime"], self.tupleCounter, None]
				row.append("http://example.org/stream/class#c" + str(const + 1))
				row.append("http://example.org/stream/class#c" + str(const + 2))
				r[p].add(tuple(row))
